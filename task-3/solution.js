 ...
 * @param {[number, number]} range text replace range
 * @returns fixer method call
 */
const fixHandler = (sortedImportsText, range) => {
  return fixer => {
    return fixer.replaceTextRange(range, sortedImportsText);
  };
};
...
const programHandler = (programNode, context) => {
  ...
  if (importsText !== sortedImportsText) {
    context.report({
      ...
      node: imports[0].node,
      fix: fixHandler(sortedImportsText, [importsStart, importsEnd])
    });
  }
};
